...
class ServiceBase {
  constructor(connection, serviceName, datasetName, idField) {
    ...
    this._getFromDs = r => r[this.datasetName][0];
  }
  ...
  getDs() {
    return this.makeRequest('GetNew' + this.datasetName, {
      ...
      return ds.parameters.ds;
    });
  }

  createByDs(ds, record) {
   ...
   */
  get(selector, returnWholeObject) {
    const queryField =
      this.idField[0].toLowerCase() + this.idField.substring(1);
    return this.makeRequest('GetByID', {
      ...
      if (returnWholeObject) {
        ...
        return result.returnObj[this.datasetName][0];
      }
